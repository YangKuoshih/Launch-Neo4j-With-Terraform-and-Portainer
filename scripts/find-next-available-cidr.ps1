...
# Get the VPC CIDR block using AWS CLI.
...
# Loop until an available /32 subnet is found.
while ($true) {
    $CANDIDATE_SUBNET = "$CURRENT_IP/32"
    
    # Check if candidate subnet is within the VPC range and not already in use.
    ...
    # Check if we've reached the end of the VPC range.
